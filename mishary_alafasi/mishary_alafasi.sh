...
#this script using to dowload alquran mp3 with the reciter is Mishary Alafasy
...
for (( c=1; c<=114; c++ ))
	...
	if (($c < 10 ))
 		then
        	urls[$c]='http://server8.mp3quran.net/afs/00'$c'.mp3'
	 	fi
	 	#creating array for link with 2 number segment
	 	if (($c >9 && $c<100 ))
	  		then
	          	urls[$c]='http://server8.mp3quran.net/afs/0'$c'.mp3'
		  	fi
		  	#creating array for link with 3 number segment
		  	if (($c >99 ))
		   		then
		           	urls[$c]='http://server8.mp3quran.net/afs/'$c'.mp3'
			   	fi
			   		done
			   	for f in "${urls[@]}"
			   		do
